...
CSIRO_seedlot$Longitude <- gsub("E", "", CSIRO_seedlot$Longitude)
...
# BV_parent <- read.csv( file ="~/RBGSyd_Technical Officer/MQuin/Parent/Iteration 1/data/metabreedingcoeff_parent_animalmdlBV_2.csv")
...
#### Actual pops name
parent_meta_pops <- parent_meta %>% dplyr::select(sample_lib_NSW, Pop, NSWID) %>% filter(!is.na(Pop)); colnames(parent_meta_pops)[2] <- "pops"
outgroups_merged_pops <- outgroups_merged %>% dplyr::select(sample_lib_NSW, pops, NSWID)
...
tissue_db_locality_NSWID <- tissue_db_full %>% dplyr::select(nswNumber, locality)
colnames(tissue_db_locality_NSWID) <- c("NSWID", "locality")
tissue_db_locality_pops <- tissue_db_full %>% dplyr::select(eventKey, locality)
...
pops_all_parents_remna_actual <- pops_all_parents_remna_actual %>%
  ...
  dplyr::select(-locality.x, -locality.y)
...
pops_all_parents_remna_actual <- left_join(pops_all_parents_remna_actual, (tissue_db_full %>% dplyr::select(locality, State)))
