## Iteration runs with 1. Top 50 BSLMM (Gapit 46), 2. Top 50 BSLMM + Top 50 epi (Gapit 47)
...
########################
# Part 1. Top 50 BSLMM (Gapit 46)

## 1. Removing bottom 50 SNPs from GAPIT 40
...
## 2. Adding Top 50 BSLMM markers - that does not overlap with Gapit 40
### Reading in data from Gapit 44 - BSLMM top 1000 SNPs
BSLMM_res <- read.csv(file = "~/RBGSyd_Technical Officer/MQuin/Seedling GWAS/Filtering/Iteration 4/gwas_7/Output/Bayesian_Sparse_Linear_Mixed_Model/COI/highest_effects/COI_BSLMM_top01%eff.csv")
genetic_data = paste0("~/RBGSyd_Technical Officer/MQuin/Seedling GWAS/Filtering/Iteration 6/data/GAPIT_44_sigSNP.hapmap.hmp.txt")
myG_44 <- read.csv(file = genetic_data, sep = "\t", header = FALSE)

BSLMM_res_rmovlp <- BSLMM_res[!(BSLMM_res$ps %in% myg40$V4),] # Filter overlapping
BSLMM_res_rmovlp_top50 <- BSLMM_res_rmovlp[1:50,] # Top 50
myG_44_top50 <- myG_44[myG_44$V4 %in% BSLMM_res_rmovlp_top50$ps,] 

## Binding
myg46 <- rbind(myg40,myG_44_top50 )
nrow(myg46)
#write.table(myg46, sep = "\t", row.names = FALSE, col.names = FALSE, file = "~/RBGSyd_Technical Officer/MQuin/Seedling GWAS/Filtering/Iteration 6/data/GAPIT_46_sigSNP.hapmap.hmp.txt")


###########################################################################################################

# Part 2. Top 50 BSLMM + Top 50 Epi (Gapit 47)

## 1. Removing bottom 100 SNPs from GAPIT 40
GWAS_res <- read.csv(file = "~/RBGSyd_Technical Officer/MQuin/Seedling GWAS/Filtering/Iteration 4/gwas/gwas_2/Output/Linear_Mixed_Model/COI/COI_20231025_215412/best_p-values/p_wald_filenamecut_top0.01.csv")
genetic_data = paste0("~/RBGSyd_Technical Officer/MQuin/Seedling GWAS/Filtering/Iteration 6/data/GAPIT_40_sigSNP.hapmap.hmp.txt")
myG <- read.csv(file = genetic_data, sep = "\t", header = FALSE)
...
myG_ordered <- myG_score[order(myG_score$p_wald,decreasing = T),]
myG_to_rm <- myG_ordered[1:100,1:2]
...
rm_MR <- epistatic_hits_ord_split %>% filter (!(V2 %in% myG$V4)) # removing epistatic hits that are already MR markers - note uses myG from above ^
...
## 3. Adding Top 50 BSLMM markers - that does not overlap with Gapit 40
### Reading in data from Gapit 44 - BSLMM top 1000 SNPs
BSLMM_res <- read.csv(file = "~/RBGSyd_Technical Officer/MQuin/Seedling GWAS/Filtering/Iteration 4/gwas_7/Output/Bayesian_Sparse_Linear_Mixed_Model/COI/highest_effects/COI_BSLMM_top01%eff.csv")
genetic_data = paste0("~/RBGSyd_Technical Officer/MQuin/Seedling GWAS/Filtering/Iteration 6/data/GAPIT_44_sigSNP.hapmap.hmp.txt")
myG_44 <- read.csv(file = genetic_data, sep = "\t", header = FALSE)
...
## Binding
myg47 <- rbind(myg40,myG_44_top50, myG_42_top50)
nrow(myg47)
write.table(myg47, sep = "\t", row.names = FALSE, col.names = FALSE, file = "~/RBGSyd_Technical Officer/MQuin/Seedling GWAS/Filtering/Iteration 6/data/GAPIT_47_sigSNP.hapmap.hmp.txt")
...
ggplot(subset(prediction_gt, RefInf==2), aes(x=COI, y=Prediction)) +
  geom_point(aes(colour = RefInf)) +
  geom_abline(intercept = 0, slope = 1) +
  geom_smooth (method = "lm", se = F, linewidth = 0.5) +
  labs (y = "Genomic Predicted COI^0.25", x = "Ground-truth COI^0.25") +
